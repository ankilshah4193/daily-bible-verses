const CONFIG = {
  ...
  DATA_VERSION: "1.0",
    ...
    /**
     * Label shown above the scripture text itself.
     * Scripture is always in English in the JSON; the label adapts to the
     * user's chosen UI language so the heading reads naturally.
     */
    SCRIPTURE_LABELS: {
    ...
    /**
     * Label shown above the reflection text.
     */
    REFLECTION_LABELS: {
  ...
  /*
   * We keep an array `unseenBag` of indices not yet shown in this cycle.
   * On each open: pop a random element, show it, save the rest.
   * When the bag empties, refill it with all 1089 indices and reshuffle.
   * `cyclesCompleted` tracks how many full passes through all verses
   * the user has done — useful for the progress display.
   */
  ...
  async function pickNextVerseState() {
    const state = await storageGet(["unseenBag", "cyclesCompleted"]);
    ...
    // First run, corrupted state, or stale bag from an older TOTAL_VERSES
    if (!bag || bag.length === 0 || bag.some((n) => n >= CONFIG.TOTAL_VERSES || n < 0)) {
      if (bag && bag.length === 0) cyclesCompleted += 1; // finishing a cycle counts
      bag = buildFreshBag();
    }
    ...
    await storageSet({ unseenBag: bag, cyclesCompleted });
  ...
  function renderVerse(verse, index, language, state) {
    ...
    // Header — brand name in chosen UI language
    elements.bibleTitle.textContent = trans.brand;
  
    // Reference: e.g. "Psalms 23:1"
    ...
    // Scripture text — always English (`english_text`)
    ...
    // Reflection — top-level language keys (english, spanish, ..., mandarin_chinese)
    // Fall back to English if the chosen language is missing.
